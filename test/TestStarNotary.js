...
it('can add the star name and star symbol properly', async() => {
    ...
    await instance.createStar('Star Name', 100, {from: user1});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar("Star 1", "101", {from: user1});
    await instance.createStar("Star 2", "102", {from: user2});
    let ownerBeforExchangeOfStar1 = await instance.ownerOf("101");
    assert.equal(ownerBeforExchangeOfStar1, user1);
    let ownerBeforExchangeOfStar2 = await instance.ownerOf("102");
    assert.equal(ownerBeforExchangeOfStar2, user2);

    await instance.exchangeStars("101", "102", {from: user1});

    let ownerAfterExchangeOfStar1 = await instance.ownerOf("101");
    assert.equal(ownerAfterExchangeOfStar1, user2);
    let ownerAfterExchangeOfStar2 = await instance.ownerOf("102");
    assert.equal(ownerAfterExchangeOfStar2, user1);
});
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar("Star 1", "103", {from: user1});
    let ownerBeforTransfer = await instance.ownerOf("103");
    assert.equal(ownerBeforTransfer, user1);

    await instance.transferStar(user2, "103", {from: user1});
    let ownerAfterTransfer = await instance.ownerOf("103");
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    await instance.createStar("Star 1", "104", {from: user1});
    let name = await instance.lookUptokenIdToStarInfo("104");
